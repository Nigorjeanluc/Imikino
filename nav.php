<nav id="menu" class="navbar container">
		<div class="navbar-header">
			<button type="button" class="btn btn-navbar navbar-toggle" data-toggle="collapse" data-target=".navbar-ex1-collapse"><i class="fa fa-bars"></i></button>
			<a class="navbar-brand" href="index.php">
				<div style="font-family:cursive;font-size:20px;font-style:italic;text-shadow: 5px 4px 4px grey;" class="logo">
					<span style="color:lightskyblue">Imi</span><span style="color:gold">kino</span><span style="color:seagreen">.net</span>
				</div>
			</a>
		</div>
		<div class="collapse navbar-collapse navbar-ex1-collapse">
			<ul class="nav navbar-nav">
				<li><a href="index.php">Home</a></li>
				<li><a href="rwanda.php">Rwanda</a></li>
				<li><a href="africa.php">Africa</a></li>
				<li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown">Ibwotamasimbi <i class="fa fa-arrow-circle-o-down"></i></a>
					<div class="dropdown-menu">
						<div class="dropdown-inner">
							<ul class="list-unstyled">
								<li><a href="england.php">England</a></li>
								<li><a href="spain.php">Spain</a></li>
								<li><a href="france.php">France</a></li>
								<li><a href="italy.php">Italy</a></li>
								<li><a href="germany.php">Germany</a></li>
								<li><a href="america.php">America</a></li>
								<li><a href="asia.php">Asia</a></li>
								<li><a href="other.php">Ahasigaye hose</a></li>
							</ul>
						</div>
					</div>
				</li>
				
				<li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown">More Sports <i class="fa fa-arrow-circle-o-down"></i></a>
					<div class="dropdown-menu" style="margin-left: -203.625px;">
						<div class="dropdown-inner">
							<ul class="list-unstyled">
								<li><a href="basketball.php">Basketball</a></li>
								<li><a href="football.php">Football</a></li>
								<li><a href="volleyball.php">Volleyball</a></li>
							</ul>
							<ul class="list-unstyled">
								<li><a href="rugby.php">Rugby</a></li>
								<li><a href="handball.php">Handball</a></li>
								<li><a href="swimming.php">Swimming</a></li>
							</ul>
							<ul class="list-unstyled">
								<li><a href="cricket.php">Cricket</a></li>
								<li><a href="cycling.php">Cycling</a></li>
								<li><a href="athletic.php">Athletic sports</a></li>
							</ul>
						</div>
					</div>
				</li>
			</ul>
			<ul class="list-inline navbar-right top-social">
				<li><a href="https://www.facebook.com/yaminilivecom-331259140367167/?ref=aymt_homepage_panel" target="_blank" rel="nofollow"><i class="fa fa-facebook"></i></a></li>
				<li><a href="https://twitter.com/ImikinoCom" target="_blank" rel="nofollow"><i class="fa fa-twitter"></i></a></li>
				<li><a href="#"><i class="fa fa-instagram"></i></a></li>
				<li><a href="#"><i class="fa fa-google-plus-square"></i></a></li>
				<li><a href="#"><i class="fa fa-whatsapp"></i></a></li>
				<li><a href="admin/login.php" target="_blank"><i class="fa fa-user"></i></a></li>
			</ul>
		</div>
	</nav>